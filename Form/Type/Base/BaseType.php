...
namespace Parabol\AdminCoreBundle\Form\Type\Base;
...
trait BaseType
{
 ...
 //    }
	...
	protected function getOptionsTranslations(array $builderOptions = array())
    {
        ...
        if(isset($result['fields'])) $result['fields'] = array_merge_recursive($default_fields, $result['fields']);
...
//                 # query_builder: function (\Parabol\AdminCoreBundle\Entity\PageRepository $er) { return $er->findAllEnabledWithoutProperty($this->getData()); }   
    ...
    protected function getDefaultTranslationFieldsSetting()
    {
        ...
        { 
            $result['content'] = array( 'field_type' => $this->getTypeContent(), 'config' => $this->getCKEditroDefaultConfig(), 'plugins' => $this->getCKEditroDefaultPlugins(), 'attr' => array('style' => 'height: 600px'));
        }
    ...
    protected function canDisplayFilesUpdatedAt()
    {
        return false;
    }
    ...
    {
        if ($this->canDisplayFilesUpdatedAt()) {
    ...
    protected function resolveOptions($name, array $fieldOptions, array $builderOptions = array(), $optionsClass = null)
    {
        ...
        // {
        //     var_dump($name, $this->currentType, $fieldOptions);
        //     die();
        // }
